...
import { GramDefaultError } from "../models/errors/gram-default-error.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | GramDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | GramDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new GramDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new GramDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
